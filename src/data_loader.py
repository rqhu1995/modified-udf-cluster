...
def load_data(config: dict) -> tuple[dict, dict, pd.DataFrame]:
    ...
    station_df = station_df[
        ["idx"] + [col for col in station_df.columns if col != "idx"]
    ]
    if len(station_df) < params["network_size"]:
        ...
        station_df = sample_stations(
            params["network_size"],
            station_df,
            config["surplus_ratio"],
            config["deficit_ratio"],
            config["random_state"],
        )
    ...
    params["t_ij"] = pd.read_csv(
        config["travel_times_path"], header=0, index_col=0
    ).to_numpy()

    transdelta_df = pd.read_csv(config["delta_marginal_path"])
    params["Delta_ij^m"] = transdelta_df.set_index(
        ["from_surplus", "to_deficit", "transfers"]
    )["Delta UDF"].to_dict()

    max_transfer_idx = transdelta_df.groupby(["from_surplus", "to_deficit"])[
        "transfers"
    ].idxmax()
    max_transfer_df = transdelta_df.loc[max_transfer_idx].reset_index(drop=True)
    params["U_ij"] = max_transfer_df.set_index(["from_surplus", "to_deficit"])[
        "transfers"
    ].to_dict()
    ...
    sets = {
        "V": sorted(
            params["s^0"].keys()
        ),  # Use keys from data; assumes 1 to network_size
    ...
    dict_B = {
        j: sum(params["t_ij"][i, j] + 2 * params["t_ij"][j, 0] for i in sets["V"])
        for j in sets["V"]
    }
...
def sample_stations(
    ...
    surplus_stations = station_df[
        station_df["CurrentInventory"] > station_df["Optimal Inventory"]
    ]
    deficit_stations = station_df[
        station_df["CurrentInventory"] < station_df["Optimal Inventory"]
    ]
    balanced_stations = station_df[
        station_df["CurrentInventory"] == station_df["Optimal Inventory"]
    ]
    ...
    sampled_surplus = (
        surplus_stations.sample(n=num_surplus, random_state=random_state)
        if not surplus_stations.empty
        else pd.DataFrame()
    )
    sampled_deficit = (
        deficit_stations.sample(n=num_deficit, random_state=random_state)
        if not deficit_stations.empty
        else pd.DataFrame()
    )
    sampled_balanced = (
        balanced_stations.sample(n=num_balanced, random_state=random_state)
        if not balanced_stations.empty
        else pd.DataFrame()
    )
