...
class MILPModel:
    def __init__(self, sets: dict, params: dict):
        ...
        self.a = self._add_a_variables(sets)
    ...
    def _add_x_variables(self, sets, params):
        ...
        indices = [
            ...
            for j in sets["V^-"]
            for v in sets["C"]
            for m in range(1, U.get((i, j), 0) + 1)
        ]
    ...
    def _add_a_variables(self, sets):
        # station is active in v iff it participates in any transfer (in or out)
        return self.model.addVars(
            [(i, v) for i in sets["V"] for v in sets["C"]], vtype=GRB.BINARY, name="a"
        )
    ...
    def _set_objective(self, sets, params):
        ...
        obj = quicksum(
            params["Delta_ij^m"][(i, j, m)] * self.x[i, j, v, m]
            for (i, j, v, m) in indices
        )
    ...
    def _add_constraints(self, sets, params):
        ...
        self.model.addConstrs(
            quicksum(self.z[i, v] for v in sets["C"]) == 1 for i in sets["V"]
        )
        # eq (4) surplus station i only in cluster v would transfer of ijvm happen
        self.model.addConstrs(
            self.x[i, j, v, m] <= self.z[i, v]
            for i in sets["V^+"]
            for j in sets["V^-"]
            for v in sets["C"]
        ...
        self.model.addConstrs(
            ...
            <= (params["s^*"][j] - params["s^0"][j])
            for j in sets["V^-"]
        )
        # eq (8.0) MSS estimation for the travel time
        self.model.addConstrs(
            self.s[v]
            >= quicksum(params["t_ij"][i, k] * self.a[i, v] for i in sets["V"])
            + 2 * params["t_ij"][0, k]
            - params["B"][k] * (1 - self.a[k, v])
            for k in sets["V"]
            for v in sets["C"]
        )
        # eq(8) total time of each cluster less and equal to loading time plus the maximum spanning star estimation
        for v in sets["C"]:
            loading_time = quicksum(
                params["t_load"] * self.x[i, j, v, m]
                for i in sets["V^+"]
                for j in sets["V^-"]
                for m in range(1, params["U_ij"].get((i, j), 0) + 1)
            )
        ...
        self.model.addConstrs(
            self.x[i, j, v, m + 1] <= self.x[i, j, v, m]
            for i in sets["V^+"]
            for j in sets["V^-"]
            for v in sets["C"]
            for m in range(1, params["U_ij"].get((i, j), 0))
        )

        # active only if in the cluster (tightening, keeps a clean)
        self.model.addConstrs(
            self.a[i, v] <= self.z[i, v] for i in sets["V"] for v in sets["C"]
        )

        # trigger activity when any transfer touches i in v
        # (outgoing for surplus stations)
        self.model.addConstrs(
            quicksum(
                self.x[i, j, v, m]
                for j in sets["V^-"]
        ...
        # (incoming for deficit stations)
        self.model.addConstrs(
            quicksum(
                self.x[h, i, v, m]
                for h in sets["V^+"]
                for m in range(1, params["U_ij"].get((h, i), 0) + 1)
            )
            >= self.a[i, v]
            for i in sets["V^-"]
            for v in sets["C"]
        ...
        # (optional tightening so x cannot touch an inactive station)
        self.model.addConstrs(
            self.x[i, j, v, m] <= self.a[i, v]
            for i in sets["V^+"]
            for j in sets["V^-"]
            for v in sets["C"]
            for m in range(1, params["U_ij"].get((i, j), 0) + 1)
